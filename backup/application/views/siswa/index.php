<div>
		<ul class="breadcrumb">
			...
			<li>
				<a href="<?=site_url()?>siswa">Siswa</a><span class="divider">/</span>
			</li>
			<li>
				Data Siswa
			</li>
...
$(document).ready(function(){
	$('#isi').load('<?=site_url()?>siswa/dataSiswa');
	
	$('#focusedInput').keyup(function(){
		var x=$(this).val();
		$('#isi').load('<?=site_url()?>siswa/dataSiswa/'+x);
	});

	$('#Yes').click(function(){
		var va=$('#iddd').val();
		location.href="<?php echo site_url();?>siswa/delete/"+va;
	});
});
...
function del(id)
{
	...
	$('#isiModalConfirm').html('<h3>Apakah Anda Yakin Mengahpus Data Siswa Ini ?</h3>');
	$('#iddd').val(id);
}
</script>